...
class LLMConfig(BaseModel):
    ...
    def __init__(self, **kwargs):
        ...
        if 'model_name' not in kwargs:
            ...
            if env_model_name:
                kwargs['model_name'] = env_model_name

        if 'api_key' not in kwargs:
            env_api_key = os.getenv('DOCS_TO_EVAL_API_KEY')
            if env_api_key:
        ...
        if 'base_url' not in kwargs:
            env_base_url = os.getenv('DOCS_TO_EVAL_BASE_URL')
            if env_base_url:
...
class ConfigManager:
    ...
    def update_from_env(self) -> None:
        ...
        Useful after .env file changes and os.environ updates.
        """
        # Create a new LLMConfig which will re-read env vars
        new_llm_config = LLMConfig()
        # Merge this new LLMConfig into the existing config
        self.config.llm = new_llm_config
        # Re-create other parts of the config if they also need to re-read env
        # For now, only LLMConfig has specific env reading logic in its __init__
        self.config.generation = GenerationConfig()
        self.config.verification = VerificationConfig()
