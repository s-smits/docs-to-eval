...
async def upload_corpus_file(file: UploadFile = File(...), name: Optional[str] = Form(None)):
    ...
    try:
        ...
        if len(text) > MAX_TEXT_LENGTH:
            raise HTTPException(status_code=413, detail=f"Text content too long")
...
async def upload_multiple_files(
    ...
    try:
        ...
        for file in files:
            ...
            if file_extension not in supported_extensions: continue
            try:
                content = await file.read()
        ...
        if not file_contents: raise HTTPException(status_code=400, detail="No valid text files found")
        ...
        if isinstance(min_tokens, int): chunking_config.min_token_size = clamp(min_tokens, 500, 8192)
        if isinstance(target_tokens, int): chunking_config.target_token_size = clamp(target_tokens, 500, 8192)
        if isinstance(max_tokens, int): chunking_config.max_token_size = clamp(max_tokens, 500, 8192)
        if isinstance(overlap_tokens, int): chunking_config.overlap_tokens = clamp(overlap_tokens, 0, 1024)
        chunks = create_smart_chunks_from_files(file_contents, chunking_config)
        combined_text = ""
        chunk_info = []
        for i, chunk in enumerate(chunks):
            if combined_text: combined_text += f"\n\n{'='*80}\nCHUNK {i+1} (from {len(chunk['file_sources'])} files)\n{'='*80}\n\n"
