...
class TestFullAgenticFlow:
    ...
    async def test_question_generation_from_chunks(self, sample_corpus, pipeline_config, mock_llm_pool):
        ...
        print(f"✅ Question generation validation passed")
        return items
    ...
    async def test_openrouter_integration(self, sample_corpus):
        ...
        gemini_llm = OpenRouterInterface(OpenRouterConfig(model="google/gemini-2.5-flash"))
...
if __name__ == "__main__":
    ...
    async def run_async_tests():
        ...
        items = await test.test_question_generation_from_chunks(corpus, pipeline_cfg, llm_pool)
        
        # Test full pipeline
        results = await test.test_full_pipeline_with_evaluation(corpus, pipeline_cfg, llm_pool)
        
        # Test metrics
        metrics = await test.test_pipeline_metrics_and_reporting(corpus, pipeline_cfg, llm_pool)
